  ...
  factory StolModel.fromJson(Map<String, dynamic> json) {
    return StolModel(
      ...
      number: json['number'] is int
          ...
          int.tryParse(json['name']?.replaceAll(RegExp(r'[^0-9]'), '')) ??
          0,
      status: json['status'],
      capacity: json['capacity'] ?? 4,
      isActive: json['is_active'],
      position: Offset.zero,
      size: Size((json['capacity'] ?? 4) * 20.0, (json['capacity'] ?? 4) * 20.0),
...
class _StollarniJoylashuvState extends State<StollarniJoylashuv> {
  ...
  final StolController _controller = StolController();
  bool isLoading = true;
  ...
  Future<void> _fetchTables() async {
    try {
      final fetchedTables = await _controller.fetchTables();
      final prefs = await SharedPreferences.getInstance();
      final cachedPositions = prefs.getString('table_positions');
      Map<String, dynamic> positionMap = {};
      ...
      setState(() {
        tables = fetchedTables.map((table) {
          if (positionMap.containsKey(table.id)) {
            final posData = positionMap[table.id];
            table.position = Offset(
              posData['position']['dx'],
            ...
            table.position = Offset(50.0 + (index % 5) * 150, 50.0 + (index ~/ 5) * 150);
          }
          return table;
        }).toList();
        isLoading = false;
  ...
  Future<void> _cachePositions() async {
    final prefs = await SharedPreferences.getInstance();
    final positionMap = {
      for (var table in tables) table.id: table.toJson(),
    };
    await prefs.setString('table_positions', json.encode(positionMap));
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Pozitsiyalar saqlandi')),
    );
  }
        ...
        onTap: () {
          showDialog(
            ...
            builder: (context) => AlertDialog(
              ...
              content: Text('${table.name} stolini o\'chirishni xohlaysizmi?'),
              actions: [
                TextButton(
