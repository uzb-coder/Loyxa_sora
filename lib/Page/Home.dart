...
class _PosScreenState extends State<PosScreen> {
  ...
  void _showOrderScreenDialog(String? tableName) {
    showDialog(
      context: context,
      barrierDismissible: false, // Loading paytida dialog yopilmasin
      builder: (BuildContext context) {
        return Dialog.fullscreen(child: OrderScreenContent(tableId: tableName));
      },
    );
  }
  ...
  Future<void> _printCheck(Map<String, dynamic> order) async {
    ...
    try {
      ...
      receipt.writeln(centerText('Stol: $_selectedTableName', 32));
        ...
        return AlertDialog(
          ...
          title: Column(
            children: [
              ...
              SizedBox(
                height: baseFontSize * 0.6,
              ), // One line spacing after restaurant name
            ],
          ),
          content: Container(
            width: 320,
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(
                        height: baseFontSize * 0.6,
                      ), // One line spacing after order number
                      Text(
                        'Stol: $_selectedTableName',
                        style: TextStyle(
                          fontSize: baseFontSize * 0.95,
                          fontFamily: 'Courier',
                          fontWeight: FontWeight.w600,
                      ...
                      SizedBox(
                        height: baseFontSize * 0.6,
                      ), // One line spacing after table number
                      Text(
                        'Vaqt: ${DateFormat('d MMMM yyyy, HH:mm', 'uz').format(DateTime.now())}',
                        style: TextStyle(
                          fontSize: baseFontSize * 0.95,
                      ...
                      SizedBox(
                        height: baseFontSize * 0.6,
                      ), // One line spacing before divider
                      Divider(
                        thickness: 1.5,
                      ...
                      SizedBox(
                        height: baseFontSize * 0.3,
                      ), // One line spacing before products header
                      Text(
                        'Mahsulotlar:',
                        style: TextStyle(
                      ...
                      SizedBox(
                        height: baseFontSize * 0.3,
                      ), // One line spacing after products header
                      Table(
                        columnWidths: const {
                        ...
                        children:
                            (order['items'] as List)
                                .map((item) {
                                  String name =
                                      item['name'].toString().length > 18
                                          ? item['name'].toString().substring(
                                            0,
                                            18,
                                          )
                                          : item['name'].toString();
                                  String price =
                                      '${item['price'].toInt()} so\'m';
                                  return TableRow(
                                    children: [
                                      Padding(
                                        padding: EdgeInsets.symmetric(
                                          vertical: baseFontSize * 0.15,
                                        ),
                                        child: Text(
                                          name,
                                          style: TextStyle(
                                    ...
                                    if (index <
                                      ...
                                      TableRow(
                                        children: [
                                          SizedBox(
                                            height: baseFontSize * 0.6,
                                          ), // One line spacing after each product
                      ...
                      SizedBox(
                        height: baseFontSize * 0.3,
                      ), // One line spacing after product list
                      Divider(
                        thickness: 1.5,
                        height: baseFontSize * 1.2,
                        color: Colors.teal.shade100,
            ...
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                if (_selectedTableName != null) {
                  _showOrderScreenDialog(_selectedTableName);
                } else {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
  ...
  Widget _buildTableCard(
    ...
    final isPending = table['status'] == 'pending';
    final isSelected = table['name'].toString() == _selectedTableName;

    return InkWell(
      onTap: () {
        _handleTableTap(table['name'].toString());
      },
      borderRadius: BorderRadius.circular(12),
      child: AnimatedContainer(
        ...
        child: Column(
          ...
          children: [
            ...
            if (isSelected)
              Container(
                margin: EdgeInsets.only(top: fontSize * 0.3),
                padding: EdgeInsets.symmetric(
  ...
  Widget _buildOrderDetails(double baseFontSize, BoxConstraints constraints) {
    final order = _orders[_selectedTableName];
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Text(
            key,
            style: TextStyle(color: Colors.grey[700], fontSize: fontSize * 0.9),
          ),
          const Spacer(),
          Text(
            value,
...
class _OrderScreenContentState extends State<OrderScreenContent> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      // Kategoriyalarni yuklash - timeout bilan
      List<Category> categories = [];
      try {
        categories = await _categoryController.fetchCategories().timeout(
          Duration(seconds: 5),
        );
      } catch (e) {
        print("⚠️ Kategoriyalar API xatoligi: $e");
        // Test kategoriyalari bilan ishlash
        categories = _getTestCategories();
      }

      // Mahsulotlarni yuklash - timeout bilan
      List<Ovqat> products = [];
      try {
        products = await _productController.fetchProducts().timeout(
          Duration(seconds: 5),
        );
      } catch (e) {
        print("⚠️ Mahsulotlar API xatoligi: $e");
        // Test ma'lumotlari bilan ishlash
        products = _getTestProducts();
      }
  ...
  List<Category> _getTestCategories() {
    return [
      Category(
        id: '1',
        title: 'Ichimliklar',
        printerName: 'ichimlik',
        printerIp: '192.168.1.128',
        subcategories: ['gazli', 'gazsiz'],
      ),
      Category(
        id: '2',
        title: 'Shirinliklar',
        printerName: 'shirinlik',
        printerIp: '192.168.0.106',
        subcategories: ['Tort', 'Perojni', 'Vafli', 'Pechini'],
      ),
      Category(
        id: '3',
        title: 'Taomlar',
        printerName: 'taom',
        printerIp: '192.168.0.106',
        subcategories: ['Osh', 'Shashlik', 'Salat'],
      ),
    ];
  }

  // Test mahsulotlari
  List<Ovqat> _getTestProducts() {
    return [
      Ovqat(
        id: '1',
        name: 'Cola',
  ...
  Widget build(BuildContext context) {
    ...
    final padding = (screenWidth * 0.03).clamp(
      8.0,
      16.0,
    ); // Min padding for small screens
      ...
      children: [
        Row(
          children: [
            ...
            Text(
              widget.tableId != null
                  ? "Stol ${widget.tableId} • Yangi hisob"
                  : "Yangi hisob",
              style: TextStyle(
                fontSize: fontSize,
                fontWeight: FontWeight.w600,
    ...
    if (_error != null) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.error, color: Colors.white, size: fontSize * 2),
            SizedBox(height: padding * 0.5),
            Text(
              'Xatolik: $_error',
            ...
            SizedBox(height: padding * 0.5),
            ElevatedButton(onPressed: _loadData, child: Text('Qayta urinish')),
          ],
        ),
      );
    }
  ...
  Widget _buildProductCard(Ovqat product, double fontSize, double padding) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          border: Border.all(
            color: isInCart ? Colors.teal.shade700 : Colors.grey.shade200,
            width: 1,
          ),
          borderRadius: BorderRadius.circular(8),
          boxShadow: [
    ...
    return Row(
      ...
      children: [
        ...
        ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(
            backgroundColor:
                isCartEmpty ? Colors.grey.shade300 : Colors.teal.shade700,
            foregroundColor: Colors.white,
            padding: EdgeInsets.symmetric(
              horizontal: padding * 1.0,
              vertical: padding * 0.4,
